...
class Character extends FunkinSprite implements IBeatReceiver implements IOffsetCompatible
{
	...
	public function applyXML(xml:Access) {
		...
		if (xml.x.exists("hasAnimIcon")) hasAnimIcon = (xml.x.get("hasAnimIcon") == "true");
	...
	public static function getHasAnimIcon(?curCharacter:String) {
		if(curCharacter == null) return false;
		...
		while(true) {
			switch (curCharacter) {
				...
				default:
					...
					if (xml.x.exists("hasAnimIcon")) isAnim = (xml.x.get("hasAnimIcon") == "true");
				}
			break;
		}
		return isAnim;
